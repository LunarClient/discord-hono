...
export class Components {
   ...
   */
  row = (
    ...e: (Button<any> | Select<any> | APIMessageActionRowComponent)[]
  ) => {
    if (this.#components.length >= 5)
      console.warn("You can have up to 5 Action Rows per message");
    this.#components.push({
      type: 1,
      components: e.map((component) =>
...
type ButtonStyle =
  ...
  | "SKU";
export class Button<
  T extends ButtonStyle = "Primary"
> extends Builder<APIButtonComponent> {
  #style: ButtonStyle;
  #uniqueStr = "";
  #assign = (
    method: string,
    doNotStyle: ButtonStyle[],
    obj: Partial<APIButtonComponent>
  ) => {
  ...
  constructor(
    str: string,
    label: T extends "SKU" ? "" | undefined : string,
  ...
  custom_id = (e: T extends "Link" | "SKU" ? undefined : string) =>
    this.#assign("custom_id", ["Link", "SKU"], {
      custom_id: this.#uniqueStr + e,
    });
  /**
   * available: ALL
...
type SelectComponent =
  ...
  | APIChannelSelectComponent;
export class Select<
  T extends SelectType = "String"
> extends Builder<SelectComponent> {
  #type: SelectType;
  #uniqueStr = "";
  #assign = (
    method: string,
    doType: SelectType[],
    obj: Partial<SelectComponent>
  ) => {
  ...
  options = (...e: T extends "String" ? APISelectMenuOption[] : undefined[]) =>
    this.#assign("options", ["String"], {
      options: e as APISelectMenuOption[],
    });
  /**
   * available: Channel
   *
