...
type Variables<
  P extends GetPath | PutPath | PostPath | PatchPath | DeletePath
> = P extends
  | "/channels/{channel.id}/messages"
  | "/channels/{channel.id}/messages/bulk-delete"
  ? [string]
...
export class Rest {
   ...
   */
  put = <P extends PutPath>(path: P, variables: Variables<P>) =>
    this.#fetch(path, variables, "PUT");
  /**
   * @param {string} path Official document path
   * @param {string[]} variables Variable part of official document path
   * @param data
   ...
   * @returns {Promise<Response>}
   */
  delete = <P extends DeletePath>(path: P, variables: Variables<P>) =>
    this.#fetch(path, variables, "DELETE");
}
