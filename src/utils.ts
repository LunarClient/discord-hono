  ...
  match(key: K) {
    if (typeof key === "string") {
      ...
      if (keys.some((k) => k instanceof RegExp))
        for (const k of keys)
          if (k === key || (k instanceof RegExp && k.test(key)))
            return this.get(k);
    }
    return this.get(key);
  }
...
export const prepareData = <T extends CustomCallbackBase>(
  data: CustomCallbackData<T>
) => {
  ...
  if (components)
    data = {
      ...data,
      components: "toJSON" in components ? components.toJSON() : components,
    };
  if (embeds)
    data = {
      ...data,
...
export const formData = <T extends CustomCallbackBase>(
  data?: CustomCallbackData<T>,
  file?: FileData
) => {
  const body = new FormData();
  if (data && Object.keys(data).length !== 0)
    body.append("payload_json", JSON.stringify(prepareData(data)));
  if (Array.isArray(file))
    for (let i = 0, len = file.length; i < len; i++)
      body.append(`files[${i}]`, file[i].blob, file[i].name);
